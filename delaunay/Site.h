...
{
public:
	Site(Point *p, int index, float weight, unsigned color);
	~Site( );

	static void clean();

	static Site *create(Point *p, int index, float weight, unsigned color);
	void dispose( );
...
private:
	/**
		...
		*
		*/
	Site *init(Point *p, int index, float weight, unsigned color);
