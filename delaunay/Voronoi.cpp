...
const Point *Voronoi::nearestSitePoint(/* proximityMap:BitmapData,*/ float x, float y)
{
...
template<>
float Voronoi::compareByYThenX(const Site *s1, Site *s2)
{
	if( s1->y( ) < s2->y( ) )
		return -1;
	if( s1->y( ) > s2->y( ) )
...
template<>
float Voronoi::compareByYThenX(const Site *s1, Point *s2)
{
	if( s1->y( ) < s2->y )
		return -1;
	if( s1->y( ) > s2->y )
...
{
	float weight = float(rand()) / float(RAND_MAX) * 100;
